...
public class Weapon : EquipmentItem
{
    ...
    public class AttackData
    {
        ...
        public int AddDamage(StatSystem.DamageType damageType, int amount)
        {
            ...
            if (damageType == StatSystem.DamageType.Physical)
            {
                ...
                if (m_Source != null)
                    addedAmount += Mathf.FloorToInt(addedAmount * m_Source.Stats.CurrentStats.strength * 0.01f);

                //each poitn of defense remove 1 damage, with a minimum of 1 damage
                addedAmount = Mathf.Max(addedAmount - m_Target.Stats.CurrentStats.defense, 1);
            }

            //we then add boost per damage type. Not this is called elementalBoost, but physical can also be boosted
            if (m_Source != null)
                addedAmount += addedAmount * Mathf.FloorToInt(m_Source.Stats.CurrentStats.elementalBoosts[(int)damageType] / 100.0f);

            //Then the elemental protection that is a percentage
            addedAmount -= addedAmount * Mathf.FloorToInt(m_Target.Stats.CurrentStats.elementalProtection[(int)damageType] / 100.0f);
    ...
    [Header("Stats")]
    public Stat Stats = new Stat() { Speed = 1.0f, MaximumDamage = 1, MinimumDamage = 1, Crit = 1 };

    public List<WeaponAttackEffect> AttackEffects;

    public void Attack(CharacterData attacker, CharacterData target, int currentDmg, bool dealthBlow = false, bool crit = false, bool dealMoreDamageBasedOnHp = false)
    {
        AttackData attackData = new AttackData(target, attacker);

        int damage = currentDmg + Random.Range(Stats.MinimumDamage, Stats.MaximumDamage + 1);
        ...
        target.Damage(attackData.GetFullDamage(), dealthBlow, crit, dealMoreDamageBasedOnHp);
    ...
    {
        if (Vector3.SqrMagnitude(attacker.transform.position - target.transform.position) < Stats.Crit * Stats.Crit)
        {
            return true;
        }
    ...
    public int CurrentMinimumDmg => (int)(Stats.MinimumDamage * Mathf.Pow(1.1f, Level));
    public int CurrentMaximumDmg => (int)(Stats.MaximumDamage * Mathf.Pow(1.1f, Level));
    public int CurrentCrit => (int)(Stats.Crit * Mathf.Pow(1.03f, Level));

    public override string GetDescription()
    {
        //string desc = base.GetDescription();

        var desc = $"{CurrentMaximumDmg - CurrentMinimumDmg}\n";
        desc += "0\n";
        desc += "0\n";
        desc += $"{Stats.Speed}\n";
        desc += $"{CurrentCrit}\n";
    ...
    {
        return Random.Range(CurrentMinimumDmg, CurrentMaximumDmg);
    }
}
