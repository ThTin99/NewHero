...
public class StatSystem
{
    ...
    {
        Physical,   //based on strength and defense
        Fire,       //25% strength for 8 s
        Cold,       //apply slow effect 50% for 2 s
        Electric,   //50% strength
        Corrode     //70% strength until death
    ...
    public class Stats
    {
        ...
        [Range(0, 100)]
        public short healingEffectiveness;
        [Range(0, 100)]
        public short attackSpeedBoost;
        [Range(0, 100)]
        public short movementSpeedBoost;

        public int upgradeCost;

        public void Copy(Stats other, bool isHero = false)
        {
            level = isHero ? GameFlowManager.Instance.UserProfile.GetCharacterInformation(GameFlowManager.Instance.UserProfile.selectedHero).Level : other.level;
        ...
        public void Modify(StatModifier modifier)
        {
            ...
            if (modifier.ModifierMode == StatModifier.Mode.Percentage)
            {
                ...
                if (modifier.Stats.elementalProtection.Length > 0)
                {
                ...
                if (modifier.Stats.elementalBoosts.Length > 0)
                {
        ...
        public int CurrentStrength => (int)(strength * Mathf.Pow(1.1f, level));
        public int CurrentHealth => (int)(health * Mathf.Pow(1.5f, level));
        public int CurrentDefend => (int)(defense * Mathf.Pow(1.1f, level));
        public int CurrentAgi => (int)(agility * Mathf.Pow(1.1f, level));
        public int CurrentCrit => (int)(health * Mathf.Pow(1.03f, level));
    }
    ...
    public Stats CurrentStats { get; set; } = new Stats();

    public int CurrentHealth { get; set; }
    public int CurrentMaxHealth => Mathf.FloorToInt(CurrentStats.health * Mathf.Pow(1.5f, CurrentStats.level));
    public int CurrentMaxStrength => Mathf.FloorToInt(CurrentStats.strength * Mathf.Pow(1.5f, CurrentStats.level));
    public int CurrentMaxDefense => Mathf.FloorToInt(CurrentStats.defense * Mathf.Pow(1.5f, CurrentStats.level));
    public int CurrentMaxAgility => Mathf.FloorToInt(CurrentStats.agility);
    ...
    public CharacterData Owner { get => m_Owner; set => m_Owner = value; }
    ...
    {
        CurrentStats.Copy(baseStats, isHero);
    ...
    void UpdateFinalStats()
    {
        ...
        int previousMaxAgi = CurrentMaxAgility;

        CurrentStats.Copy(baseStats);

        foreach (var modifier in m_ModifiersStack)//equipment/passive goes here
        {
            CurrentStats.Modify(modifier);
        }

        foreach (var timedModifier in m_TimedModifierStack)
        {
            CurrentStats.Modify(timedModifier.Modifier);
        }

        //if we change the max health we update the current health to it's new value
        if (previousMaxHealth != CurrentMaxHealth)
        {
            float percentage = CurrentHealth / (float)previousMaxHealth;
            CurrentHealth = Mathf.RoundToInt(percentage * CurrentMaxHealth);
        }

        if (previousMaxAgi != CurrentMaxAgility)
        {
            m_Owner.ChangeAgentSpeed(CurrentMaxAgility);
        }
    }
    ...
    public void Damage(int attackData, bool deadBlow = false, bool heal = false)
    {
        int totalDamage = attackData;

        if (deadBlow == true)
        {
            DamageUI.Instance.NewDamage(0, m_Owner.transform.position, true, "Dead Blow");
